...
def es_color_cafe(color_str):
    # EL FIX ESTÁ AQUÍ: Agregamos '64' que es el código de Excel para "Sin Relleno"
    if color_str in ['00000000', '000000', 'NONE', '', 'FFFFFFFF', '00FFFFFF', '64', 'SYSTEM FOREGROUND']: 
        return False
    ...
    # Si no es ninguno de los anteriores (sin relleno, amarillo, azul), es Cancelada
    return True
...
def cargar_y_procesar_datos(archivo_subido):
    ...
    if len(filas_validas) > len(df): 
        filas_validas = filas_validas[:len(df)]
    elif len(filas_validas) < len(df): 
        filas_validas.extend([True] * (len(df) - len(filas_validas)))
...
if archivo_subido is not None:
    try:
        ...
        if df.empty:
            ...
            with st.expander("🛠️ Ver datos crudos (Depuración)"):
                st.write("Si los números no cuadran, revisa aquí cómo el código clasificó las visitas:")
                st.dataframe(df[[col_medico, 'ESTADO_VISITA', 'PRODUCTIVIDAD']].head(15))
