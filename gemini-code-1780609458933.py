...
def es_color_cafe(color_str):
    # Si está vacío, es negro/transparente o es BLANCO, NO es café (es consulta efectiva)
    if color_str in ['00000000', '000000', 'NONE', '', 'FFFFFFFF', '00FFFFFF']: 
        return False
    # Si es amarillo, NO es café
    if 'FF' in color_str and any(y in color_str for y in ['FFFF00', 'FFF2CC', 'FFFF99', 'FFFFCC', 'FFEB9C', 'FFFFC000']):
        return False
    # Si es azul, NO es café
    if es_color_azul(color_str): 
        return False
    # Si tiene cualquier otro color fuerte (rojo/café), es cancelada
    return True
...
def cargar_y_procesar_datos(archivo_subido):
    ...
    for idx, row in enumerate(hoja.iter_rows(min_row=2, max_row=max_filas, max_col=hoja.max_column)):
        ...
        # Asume que el color de cancelación se pinta en la columna 6 (índice 5, columna F)
