...
if archivo_subido is not None:
    try:
        ...
        if df.empty:
            ...
            # Encontrar columna del médico
            col_medico = 'MEDICO' if 'MEDICO' in df.columns else df.columns[0]
            
            # Limpiar columna de médico de falsos médicos (Horarios)
            ...
            # --- NUEVO: FILTRO MANUAL PARA TURNOS VACÍOS ---
            st.sidebar.header("1. Configuración de Datos")
            st.sidebar.info("Selecciona la columna que tiene el nombre o documento del paciente. Esto borrará los turnos vacíos de la gráfica.")
            
            # Sugerir automáticamente la columna si tiene la palabra PACIENTE, NOMBRE o DOC
            sugerencia_idx = 0
            ...
            col_paciente = st.sidebar.selectbox(
                "Columna de Paciente (Obligatorio)", 
                options=df.columns, 
                index=sugerencia_idx
            )
            
            # ¡ELIMINAR TURNOS VACÍOS BASADOS EN LA SELECCIÓN!
            if col_paciente:
                df = df[df[col_paciente].notna()]
            
            # Calcular Productividad sobre lo que queda
